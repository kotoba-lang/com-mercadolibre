(ns mercadolibre.main
  "Kotodama WASM entrypoint for the Mercadolibre clean-room actor (L4) — Clojure port.
  ...
  facts (`mercadolibre.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mercadolibre")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mercadolibre.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mercadolibre-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mercadolibre-compat/ready)
